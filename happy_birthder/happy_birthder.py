...
from happy_birthder.mixins import CommandsMixin, DialogsMixin
from happy_birthder.models import DB, User
...
class HappyBirthder(CommandsMixin, DialogsMixin, MeeseeksCore):
    ...
    async def check_dates(self):
        ...
        for user in users:
            ...
            fwd = user.fwd
            user_ids_for_mailing = [user.user_id for user in users if user.user_id != user_id]
            today = date.today()
            ...
            if birth_date is None:
                ...
                for user_id_for_mailing in user_ids_for_mailing:
